...
neural_3 = 35;                 % parametr okreslajacy jak dokladnie ma szukac krawedzie w obrazie badanym - parm.3 (10 - 50)
filtracja = false;               % parametr okreslajacy czy chcemy filtrowac wzorzec
neural_4_1 = 200;                % parametr okreslajacy filtracje wzorca od gory - parm.4.1 (150 - 220)
neural_4_2 = 100;               % parametr okreslajacy filtracje wzorca od dolu - parm.4.2   (50 - 150)
neural_5 = 10;                 % parametr okreslajacy roznice miedzy obrazem badanym a wzorcem - parm.5 ( 5 - 40 ) 

neural_6 = 10;                  % parametr szukaj?cy optymalnego rozmiaru wzorca - parm.6 ( 5 - 40 ) 
step_neural_6 = 5;              % skok szukania optymalnego rozmiaru szer
neural_7 = 10;
step_neural_7 = 5;              % wysokosc

neural_8 = 25;                  % parametr okreslajacy dokladnosc szukania - im wieksza tym mniej dokladne szukanie i wiecej wynikow
...
Img_badany_kolor = [Img_badany];
[high_badany, width_badany, lol] = size(Img_badany);
Img_badany = double(Img_badany);
...
for j = 1 : (high_badany - 1)
	for i = 1 : (width_badany - 1)
        ...
        if( template_value > neural_3 )
            J_krawedzie(j,i) = template_value * 1.50;
        else
            J_krawedzie(j,i) = 0;
        end
...
figure(3);
imshow(J_krawedzie);
title('Obraz badany na krawedzie');
            ...
            for y = 2 : high-1
                for x = 2 : width-1
                    if(wzorzec(y,x) > neural_4_1 )
                        wzorzec(y,x) = floor(( abs (wzorzec(y-1,x-1) + wzorzec(y,x-1) + wzorzec(y+1,x-1) + wzorzec(y-1,x) + wzorzec(y,x) + wzorzec(y+1,x) + wzorzec(y-1,x+1) + wzorzec(y,x+1) + wzorzec(y+1,x+1))) / 9 );
                    end
...
%         J_krawedzie = imread('temp.jpg');
        ...
        for j = 1 : (high_badany - high)
            for i = 1 : (width_badany - width) 

                for z = 1 : 100
                   val = rand;
                   val_x = floor( val * width ) + Parm_6;
                   
                   val = rand;
                   val_y = floor( val * width ) + Parm_7;
                   
                   if( wzorzec(val_y, val_x) > 50 ) 
                      tozsamosc_wzorca = tozsamosc_wzorca + ( wzorzec(val_y, val_x) / 255 ) ; 
                       
                      if( abs( J_krawedzie(val_y + i - 1 , val_x + j - 1) - wzorzec(val_y , val_x) ) < neural_5)
                          tozsamosc = tozsamosc + tozsamosc_wzorca;
                   end
